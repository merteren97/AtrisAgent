...
/**
 * Installs the Phase 4 project-memory API. Entry.ts registers these routes after
 * the normal /api authentication + Premium middleware has already been installed.
 */
export function installProjectMemoryRoutes(app: Express, options: ProjectMemoryRoutesOptions): void {
  ...
  app.get('/api/memory/projects', async (_req: Request, res: Response) => {
    try {
      res.json(await memory.listProjects());
    } catch (error: any) {
  ...
  app.get('/api/memory/workspaces/:workspaceId', async (req: Request, res: Response) => {
    try {
      ...
      // Resolve through a mission when possible so Phase-3 lazy historical backfill
      // runs before the Inspector displays the snapshot.
  ...
  app.get('/api/memory/projects/:projectId', async (req: Request, res: Response) => {
    try {
      res.json(await memory.getSnapshot(req.params.projectId));
    } catch (error: any) {
  ...
  app.get('/api/memory/projects/:projectId/search', async (req: Request, res: Response) => {
    try {
      ...
      const nodeTypes = String(req.query.types || '')
        .split(',')
        .map((item) => item.trim() as MemoryNodeType)
        .filter((item) => NODE_TYPES.has(item));
      const statuses = String(req.query.statuses || '')
        .split(',')
        .map((item) => item.trim() as MemoryNodeStatus)
        .filter((item) => NODE_STATUSES.has(item));
  ...
  app.post('/api/memory/projects/:projectId/nodes', async (req: Request, res: Response) => {
    try {
      ...
      const node = await memory.createManualMemory(req.params.projectId, {
        type,
        title,
        summary,
  ...
  app.patch('/api/memory/nodes/:nodeId', async (req: Request, res: Response) => {
    try {
      ...
      res.json(await memory.updateMemoryNode(req.params.nodeId, updates as any));
    } catch (error: any) {
  ...
  app.delete('/api/memory/nodes/:nodeId', async (req: Request, res: Response) => {
    try {
      await memory.deleteMemoryNode(req.params.nodeId);
  ...
  app.post('/api/memory/projects/:projectId/archive', async (req: Request, res: Response) => {
    try {
      res.json(await memory.archiveProject(req.params.projectId));
    } catch (error: any) {
      res.status(400).json({ error: error?.message || 'Failed to archive project memory.' });
    }
  });

  app.post('/api/memory/projects/:projectId/restore', async (req: Request, res: Response) => {
    try {
      res.json(await memory.restoreProject(req.params.projectId));
    } catch (error: any) {
      res.status(400).json({ error: error?.message || 'Failed to restore project memory.' });
    }
  });

  app.delete('/api/memory/projects/:projectId', async (req: Request, res: Response) => {
    try {
      await memory.deleteProjectMemory(req.params.projectId);
      res.json({ success: true });
    } catch (error: any) {
  ...
  app.post('/api/memory/projects/:projectId/export', async (req: Request, res: Response) => {
    try {
      ...
      if (!targetPath || targetPath.includes('\0') || !path.isAbsolute(targetPath)) {
        return void res.status(400).json({ error: 'A valid absolute export path is required.' });
      }
      if (path.extname(targetPath).toLowerCase() !== '.json') {
        return void res.status(400).json({ error: 'Memory backups must use the .json extension.' });
      }
      const parent = path.dirname(targetPath);
      if (!fs.existsSync(parent) || !fs.statSync(parent).isDirectory()) {
        return void res.status(400).json({ error: 'The selected backup directory does not exist.' });
      }

      const snapshot = await memory.getSnapshot(req.params.projectId);
      const payload = JSON.stringify({
        format: 'atris-project-memory',
        version: 1,
      ...
      if (bytes > MAX_EXPORT_BYTES) {
        return void res.status(413).json({ error: 'Project memory backup is too large for the current export limit.' });
      }
