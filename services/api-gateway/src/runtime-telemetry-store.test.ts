...
sqlite.exec(`CREATE TABLE runtime_telemetry (
  id TEXT PRIMARY KEY, mission_id TEXT NOT NULL, task_id TEXT NOT NULL, attempt_id TEXT,
  agent_instance_id TEXT NOT NULL, adapter_id TEXT NOT NULL, account_profile_id TEXT,
  outcome TEXT NOT NULL, usage_available INTEGER NOT NULL, usage_source TEXT NOT NULL,
...
const event = {
  ...
  attemptId: 'attempt-2', usageAvailable: true, usageSource: 'provider_reported' as const,
  durationMs: 400, retryCount: 1, workerUtilization: 0.5, timestamp: '2026-08-24T20:00:00.000Z',
};
...
assert.equal(row.retry_count, 1, 'telemetry persists retry count rather than attempt count');
assert.equal(row.attempt_id, 'attempt-2', 'telemetry persists attempt identity');
assert.equal(row.usage_available, 1, 'telemetry persists provider usage availability');
