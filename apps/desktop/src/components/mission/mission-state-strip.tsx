...
export function MissionStateStrip() {
  const { missions, activeMissionId, activeTasks, commandQueue, transportStatus, transportError, error } = useMissionStore();
  ...
  return (
    <section className="flex shrink-0 items-center gap-3 border-b border-border/70 bg-card/35 px-3 py-1.5" aria-label="Mission and connection status" role="status">
      <div className="min-w-0 flex-1 truncate text-xs font-medium" title={mission.title}>
        {mission.title}
      </div>
      <div className="flex shrink-0 items-center gap-2 text-[10px] text-muted-foreground">
        {visibleError ? (
          <span className="flex max-w-64 items-center gap-1 truncate text-destructive" title={visibleError} role="alert">
            <AlertCircle className="h-3 w-3 shrink-0" />
            <span className="truncate">{visibleError}</span>
          </span>
        ) : (
          <span className="hidden items-center gap-1 sm:flex">
            {transportStatus === 'connecting' || transportStatus === 'reconnecting' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Wifi className="h-3 w-3" />}
            {serviceOnline ? TRANSPORT_LABEL[transportStatus] : 'Service offline'}
          </span>
        )}
