        ...
        <div className="relative rounded-xl border border-border/70 bg-card/70 px-3 pb-2 pt-3 shadow-sm transition-all focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10">
          ...
          <div className="mt-2 flex items-center justify-between gap-2 border-t border-border/50 pt-2">
            <div className="flex min-w-0 items-center gap-1.5 text-[9px] text-muted-foreground">
              ...
              {resolvedReasoningLabel(reasoningLevel, selectedModelObject?.supportedReasoning || [])}
            </div>
            <Button
              size="icon"
...
function resolvedReasoningLabel(reasoningLevel: string, supported: string[]) {
  if (!reasoningLevel || reasoningLevel === 'none' || !supported.length) return null;
  return <span className="shrink-0">· {reasoningLevel}</span>;
}
...
export function ChatComposer() {
  ...
  if (!busy && activeMissionId && queuedCount > 0) {
    queueMicrotask(() => { void drainQueuedTurn(activeMissionId); });
  }
